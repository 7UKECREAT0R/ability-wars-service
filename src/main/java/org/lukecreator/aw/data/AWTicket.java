...
public abstract class AWTicket {
    ...
    public static AWTicket createBasedOnType(Type type, long id,
        ...
        switch (type) {
            ...
            default -> {
                throw new RuntimeException("Tried to create a ticket with an invalid type. (type: %d, id: %d)"
     ...
     * <p>
     * This method MUST defer a reply to send a "Bot is thinking..." message. The caller will assume that if true is returned, the deferral has gone through.
     *
    ...
    public void createOrGetChannel(JDA jda, Consumer<TextChannel> successConsumer, Consumer<Throwable> failureConsumer) {
        ...
        category.createTextChannel(ticketType.channelPrefix + this.id)
                ...
                .queue(whenDone -> {
                    ...
                    // send the message
                    action.queue(msg -> {
                        this.afterInitialMessageSent(whenDone, msg);
                    });
     ...
     * @param onSuccess If not null,
     * @throws SQLException If something went wrong in the database.
     */
    public void cleanup(JDA jda, @Nullable Consumer<JDA> onSuccess) throws SQLException {
        // remove the channel if it exists.
        Guild guild = jda.getGuildById(this.type().guildId);
        if (guild != null) {
            TextChannel ticketChannel = guild.getTextChannelById(this.discordChannelId);
            if (ticketChannel != null) {
                ticketChannel.delete().queue(whenDone -> {
                    if (onSuccess != null)
                        onSuccess.accept(jda);
                }, ignored -> { /* silently fail */ });
            }
        }

        this.isOpen = false;
        this.closeReason = "Ticket died";
