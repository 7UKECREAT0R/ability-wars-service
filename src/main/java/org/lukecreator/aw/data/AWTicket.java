...
public abstract class AWTicket {
    ...
    public static Modal tryOpenNewTicket(AWTicket.Type ticketType, Guild currentGuild, UserSnowflake ticketOwner) throws IllegalArgumentException, SQLException {
        ...
        AWTicket ticket = switch (ticketType) {
            case PlayerReport ->
    ...
    public static @NonNull AWTicket createBasedOnType(Type type, long id,
                                                      long discordChannelId, long openedTimestamp,
                                                      boolean isOpen, String closeReason,
                                                      long closedByDiscordId, JsonObject inputQuestions,
                                                      long ownerDiscordId) {
    ...
    public static AWTicket[] loadByOwner(UserSnowflake owner, int limit) throws SQLException {
        ...
        try (var statement = AWDatabase.connection.prepareStatement("SELECT ticket_id FROM tickets WHERE owner_discord_id = ? ORDER BY opened_timestamp DESC LIMIT ?")) {
    ...
    public static AWTicket[] loadByCloser(UserSnowflake closer, int limit) throws SQLException {
        ...
        try (var statement = AWDatabase.connection.prepareStatement("SELECT ticket_id FROM tickets WHERE is_open = false AND closed_by = ? ORDER BY opened_timestamp DESC LIMIT ?")) {
     ...
     * @param onFinishedLoading A callback that will be called when all the data the ticket needs to gather ahead-of-time is fully done loading.
     *                          A boolean parameter inside the consumer indicates if the loading was successful and the ticket should continue opening.
     *                          If `false`, then stop execution completely and assume that the original message has been updated with useful information.
     */
    public abstract void loadFromModalResponse(ModalInteractionEvent event, Consumer<Boolean> onFinishedLoading) throws SQLException;
